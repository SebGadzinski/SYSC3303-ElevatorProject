...
public final class Config {
	...
	static {
		for (int port = 5600; port < 5900; ++port)
			ports.add(port);
		try {
		...
		for (int floorPort = 0; floorPort < NUMBER_OF_FLOORS; floorPort++)
			FLOORS_UDP_INFO[floorPort] = new UDPInfo(localhost, getPort(), getPort());
		for (int elevatorPort = 0; elevatorPort < NUMBER_OF_ELEVATORS; elevatorPort++)
			ELEVATORS_UDP_INFO[elevatorPort] = new UDPInfo(localhost, getPort(), getPort());
	}
