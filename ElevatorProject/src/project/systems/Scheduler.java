...
public class Scheduler extends AbstractSubsystem implements Runnable {
    ...
    public Scheduler(InetAddress inetAddress, int inSocketPort, int outSocketPort) {
        ...
        for(int i = 0; i < project.Config.NUMBER_OF_ELEVATORS; i++){
            elevators.add(new SchedulerElevatorInfo(Integer.toString(i), project.Config.ELEVATORS_UDP_INFO[i], ElevatorDirection.IDLE, ElevatorDoorStatus.CLOSED, 0));
        }
    ...
    public synchronized void dispatchRequestToElevatorSubsystem(Request request, SchedulerElevatorInfo elevatorInfo) {
        ...
        System.out.println(this + " sent a request to " + elevatorInfo);
    ...
    public synchronized void dispatchRequestToFloorSubsystem(Request request, SchedulerFloorInfo floorInfo) {
        ...
        System.out.println(this + " says:");
        System.out.println(this + " sent a request to " + floorInfo);
        System.out.println(request);
        System.out.println();
    }
    ...
    private synchronized void consumeRequest(Request request) {
        switch (state){
            case DISPATCH_REQUEST_TO_SUBSYSTEM -> {
                //If a current destination for the elevator has already been picked, add it to the list requests, otherise send it to elevatorSubsystem
                if (request instanceof FileRequest) {
                    FileRequest fileRequest = (FileRequest) request;
                    SchedulerElevatorInfo elevator = selectElevator(fileRequest);
                    PersonRequest personRequest = new PersonRequest(fileRequest.getOriginFloor(), fileRequest.getDestinationFloor(), false, false);

                    elevator.addToRequests(personRequest);
                    System.out.println("Added Request to list of requests to: \n" + elevator.getSource());
                ...
                if(request.getSource().getSubsystem() == SubsystemSource.Subsystem.ELEVATOR_SUBSYSTEM){
                    ...
                    if (request instanceof ElevatorDestinationRequest) {
                        ...
                        if(elevator.getCurrentDestinationFloor() != -1){
                            ElevatorDirection direction = getDirectionFromFloor(elevator.getCurrentDestinationFloor(), elevator);
                            ...
                            if(needToOpenDoors(elevator)){
                                dispatchRequestToFloorSubsystem(new ElevatorDoorRequest(getSource(), ElevatorDoorStatus.OPENED), floors.get(elevator.getCurrentFloor()));
                                dispatchRequestToElevatorSubsystem(new ElevatorDoorRequest(getSource(), ElevatorDoorStatus.OPENED), elevator);
                            }else{
                                dispatchRequestToElevatorSubsystem(new ElevatorMotorRequest(getSource(), getDirectionFromFloor(elevator.getCurrentDestinationFloor(), elevator)) , elevator);
                            }
                        }
                        else{
                            if(elevator.getRequests().size() > 0){
                                PersonRequest closestRequest = elevator.closestRequest();
                                elevator.setCurrentDestinationFloor( closestRequest.isOriginFloorCompleted() ? closestRequest.getDestinationFloor() : closestRequest.getOrginFloor());
                     ...
                     if (request instanceof ElevatorArrivalRequest) {
                        ...
                        for(int i = 0; i < requests.size(); i++){
                            if(requests.get(i).isOriginFloorCompleted() == false && requests.get(i).getOrginFloor() == elevator.getCurrentFloor()){
                                motorRequest = new ElevatorMotorRequest(getSource(), ElevatorDirection.IDLE);
                            } 
                            if(requests.get(i).isOriginFloorCompleted() == true && requests.get(i).isDestinationFloorCompleted() == false && requests.get(i).getDestinationFloor() == elevator.getCurrentFloor()){
                                motorRequest = new ElevatorMotorRequest(getSource(), ElevatorDirection.IDLE);
                            }
                        }
                        ...
                        if(elevator.getDirection() == ElevatorDirection.IDLE){
                            dispatchRequestToFloorSubsystem(new ElevatorArrivalRequest(elevator.getSource(), elevator.getCurrentFloor(), elevator.getDirection()), floors.get(elevator.getCurrentFloor()));
                        }
                    ...
                    if (request instanceof ElevatorDoorRequest) {
                        ...
                        if(doorRequest.getRequestedDoorStatus() == ElevatorDoorStatus.CLOSED){
                            ...
                            if(elevator.getCurrentDestinationFloor() != -1){
                                ElevatorDirection elevatorDirection = getDirectionFromFloor(elevator.getCurrentDestinationFloor(), elevator);
                                if(ElevatorDirection.IDLE == elevatorDirection){
                                    System.out.println(elevator.getRequests());
                                }else{
                                    dispatchRequestToElevatorSubsystem(new ElevatorMotorRequest(getSource(), elevatorDirection), elevator);
                                }
                            }
                            else{
                                if(elevator.getRequests().size() > 0){
                                    PersonRequest closestRequest = elevator.closestRequest();
                                    int destination = closestRequest.isOriginFloorCompleted() ? closestRequest.getDestinationFloor() : closestRequest.getOrginFloor();
                                    dispatchRequestToElevatorSubsystem(new ElevatorDestinationRequest(getSource(),destination, getDirectionFromFloor(destination, elevator)) , elevator);
                                }
                            }
                        }
                        else{
                    ...
                    if (request instanceof ElevatorPassengerWaitRequest) {
                        ...
                        if(passengerWaitRequest.getState() == WaitState.WAITING){
                            dispatchRequestToElevatorSubsystem(new ElevatorPassengerWaitRequest(getSource(), passengerWaitRequest.getWaitTime(), WaitState.WAITING), elevator);
                        }
                        else{
                            ArrayList<PersonRequest> requests = elevator.getRequests();
                            ...
                            for(int i = 0; i < loopCount; i++){
                                ...
                                if(requests.get(actualIndex).isOriginFloorCompleted() == false && requests.get(actualIndex).getOrginFloor() == elevator.getCurrentFloor()){
                                    elevator.setPersonRequestOriginFloorCompleted(actualIndex, true);
                                    elevator.addPassenger();
                                    System.out.println("************************\n " + elevator.getSource() + "Added a passenger: \n" + requests.get(actualIndex));
                                } 
                                //Remove all the people coming from the elevator at this floor (people at floor)
                                if(requests.get(actualIndex).isOriginFloorCompleted() == true && requests.get(actualIndex).isDestinationFloorCompleted() == false && requests.get(actualIndex).getDestinationFloor() == elevator.getCurrentFloor()){
                                    ...
                                    numberOfDeletions++;
                                    System.out.println("************************\n " + elevator.getSource() + "Completed passengers request: \n" + personRequest);
                                }
                            }
                            dispatchRequestToFloorSubsystem(new ElevatorDoorRequest(getSource(), ElevatorDoorStatus.CLOSED), floors.get(elevator.getCurrentFloor()));
                            dispatchRequestToElevatorSubsystem(new ElevatorDoorRequest(getSource(), ElevatorDoorStatus.CLOSED), elevator);
                        }
                    }
    ...
    private boolean needToOpenDoors(SchedulerElevatorInfo elevator) {
        ...
        if(elevator.getDirection() == ElevatorDirection.IDLE){
            for(int i = 0; i < elevator.getRequests().size(); i++){
                if(elevator.getRequests().get(i).isOriginFloorCompleted() == false && elevator.getRequests().get(i).getOrginFloor() == elevator.getCurrentFloor()){
                    needToOpenDoors = true;
                } 
                if(elevator.getRequests().get(i).isOriginFloorCompleted() == true && elevator.getRequests().get(i).isDestinationFloorCompleted() == false && elevator.getRequests().get(i).getDestinationFloor() == elevator.getCurrentFloor()){
                    needToOpenDoors = true;
                }
            }
        }
    ...
    /**
     * Handles a person request by checking if any of its floor requests (origin or destination) are completed
     *
    ...
    private void handlePersonRequest(PersonRequest personRequest, SchedulerElevatorInfo elevator) {
        if(!personRequest.isOriginFloorCompleted()){
            dispatchRequestToElevatorSubsystem(new ElevatorMotorRequest(getSource(), getDirectionFromFloor(personRequest.getOrginFloor(), elevator)), elevator);
        }
        else{
            if(!personRequest.isDestinationFloorCompleted()){
                dispatchRequestToElevatorSubsystem(new ElevatorMotorRequest(getSource(), getDirectionFromFloor(personRequest.getDestinationFloor(), elevator)), elevator);
            }
        }
    }
    ...
    private ElevatorDirection getDirectionFromFloor(int destinationFloor, SchedulerElevatorInfo elevator) {
        if(destinationFloor == elevator.getCurrentFloor()) return ElevatorDirection.IDLE;
        else if(destinationFloor > elevator.getCurrentFloor()) return ElevatorDirection.UP;
    ...
    private boolean elevatorComingTowardsFloor(SchedulerElevatorInfo elevator, int destinationFloor) {
        ...
        if(elevator.getDirection() == ElevatorDirection.UP){
            if(elevator.getCurrentFloor() <= destinationFloor) return true;
        }
    ...
    private SchedulerElevatorInfo selectElevator(FileRequest fileRequest) {
        ...
        for(int i = 1;i < elevators.size(); i++){
            ...
            if(elevatorComingTowardsFloor(elevator, fileRequest.getOriginFloor())){
                if( Math.abs(fileRequest.getOriginFloor() - elevator.getCurrentFloor()) < Math.abs(fileRequest.getOriginFloor() - chosenElevator.getCurrentFloor()) 
                                                            && elevator.getPassengers() < project.Config.MAX_PASSENGERS_IN_ELEVATOR){                                     
                    //Elevator cannot have more than 5 passengers compared to chosen elevator to become chosen elevator
